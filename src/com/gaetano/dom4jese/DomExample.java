...
public class DomExample {
    ...
    public static void ReadSpecElem(String numId){
        try {
            File inputFile = new File("C:\\Users\\gaeta\\Corso\\BesidetechCorso\\test.xml");
            SAXReader reader = new SAXReader();
            Document document = reader.read(inputFile);
            ...
            Node node = (Node) document.selectNodes("/class/studente[@id =" +numId + "]" );
    ...
    public static void createElement (){
        try {
            ...
            Element root = document.addElement( "vehicle" );
            Element supercarElement = root.addElement("motorbikes")
                    .addAttribute("brand", "Ducati");

            supercarElement.addElement("motoname")
                    .addAttribute("type", "roadseries")
                    .addText("StreetFighter");

            supercarElement.addElement("motoname")
                    .addAttribute("type", "sportseries")
                    .addText("SuperSport");

            OutputFormat format = OutputFormat.createPrettyPrint();
            FileWriter writer= new FileWriter("C:\\Users\\gaeta\\Corso\\BesidetechCorso\\codice.xml");
            XMLWriter writer2 = new XMLWriter( writer, format );
            writer2.write( document );
            writer2.close();
        } catch (UnsupportedEncodingException e) {
